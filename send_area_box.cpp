...
SendAreaBox::SendAreaBox(QWidget* parent) : QGroupBox(parent)
{
	...
	//配置连接 单项窗口发送
	connect(this->single, &SingleSendWidget::signal_Send, [this](QString str) {
		QByteArray byteArray = this->getSentContent(str);
		emit this->requestToSend(byteArray);
		});

	//配置连接 多项窗口发送
	connect(this->multiple, &MultipleSendWidget::signal_Send, [this](QString str) {
		QByteArray byteArray = this->getSentContent(str);
		emit this->requestToSend(byteArray);
		});
	//配置连接 多项窗口通知
	connect(this->multiple, &MultipleSendWidget::signal_Notification, [this](QString str) {
	...
	connect(timer, &QTimer::timeout, [this]() {
		...
		if (this->mode == SendMode::Single)
			str = this->single->getUserString();
		else if (this->mode == SendMode::Multiple)
			str = this->multiple->getUserString();
		qDebug() << str;
...
void SendAreaBox::setupUi()
{
    ...
    single = new SingleSendWidget(this);

    verticalLayout->addWidget(single);

    multiple = new MultipleSendWidget(this);

    verticalLayout->addWidget(multiple);
}
...
{
	this->single->setAllowSending(a);
	this->multiple->setAllowSending(a);
}
	...
	: QWidget(parent)
{
	...
	connect(this->pushButton_Location, &QPushButton::clicked, [this]() {
		...
		{
			qDebug() << "无法打开文件夹";
			emit this->signal_Notification("找不到默认保存位置，请检查");
		}
		});
...
void MultipleSendWidget::setAllowSending(bool b)
{
	this->allowSending = b;
	//遍历切换选项窗口
