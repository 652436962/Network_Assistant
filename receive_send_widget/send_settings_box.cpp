...
#include "ui_send_settings_box.h"
	...
	, ui(new Ui::SendSettingsBox)
{
	...
	//自动发送
	connect(ui->checkBox_AutoSend, &QCheckBox::toggled, [this](bool checked) {
		emit this->signal_AutoSend(checked);
		});

	//配置连接 自动发送周期改变
	connect(ui->spinBox_AutoSendCycle, &DebouncedSpinBox::valueSettled, [this](int value) {
		emit this->requestChangeCycle(value);
		});

	// 添加发送区选项
	ui->comboBox_Option->addItem("单项发送", QVariant::fromValue<SendMode>(SendMode::Single));
	ui->comboBox_Option->addItem("多项发送", QVariant::fromValue<SendMode>(SendMode::Multiple));
	...
	connect(ui->comboBox_Option, &QComboBox::currentIndexChanged, [this]() {
		SendMode option = ui->comboBox_Option->currentData().value<SendMode>();
		emit this->changeSendArray(option);
		});

	//追加选项
	QByteArray item1("\r\n", 2);
	ui->comboBox_Append->addItem("CRLF('\\r''\\n')", item1);
	QByteArray item2("\n", 1);
	ui->comboBox_Append->addItem("LF('\\n')", item2);
	QByteArray item3("\0", 1);
	ui->comboBox_Append->addItem("'\\0'", item3);
	connect(ui->checkBox_Append, &QCheckBox::toggled, [this](bool checked) {
		QByteArray data = ui->comboBox_Append->currentData().value<QByteArray>();
		emit this->setAppend(checked, data);
		});
	connect(ui->comboBox_Append, &QComboBox::currentIndexChanged, [this]() {
		bool append = ui->checkBox_Append->isChecked();
		QByteArray data = ui->comboBox_Append->currentData().value<QByteArray>();
...
{
	return ui->checkBox_AutoSend->isChecked();
}

int SendSettingsBox::getAutoSendCycle(void) const
{
	return ui->spinBox_AutoSendCycle->value();
}
...
void SendSettingsBox::changeAccordingState(bool checked)
{
	if (checked)
	{
		ui->checkBox_AutoSend->setEnabled(true);//启用自动发送复选框
	}
	else
	{
		if (ui->checkBox_AutoSend->isChecked())
		{
			ui->checkBox_AutoSend->setChecked(false);//取消自动发送的选择
			emit this->signal_AutoSend(false);
		}
		ui->checkBox_AutoSend->setEnabled(false);//禁用自动发送复选框
	}
}
