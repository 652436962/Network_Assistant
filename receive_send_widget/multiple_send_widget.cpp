	...
	, ui(new Ui::MultipleSendWidget)
{
	...
	//切换页后 自动发送当前行归0
	connect(ui->tabWidget, &QTabWidget::currentChanged, [this]() {this->rowIndex = 0; });
...
{
	if (ui->tabWidget->count() <= 0) return QString();
...
{
	ScrollableListWidget* scrollWidget = new ScrollableListWidget(ui->tabWidget);
