...
MainWindow::~MainWindow()
{
	...
	{
		for (auto socket : this->tcpSocketsList)//断开所有客户端的连接
		{
...
void MainWindow::AsTcpServerOperation(void)
{
	if (this->tcpServer == nullptr)
	{
		...
		connect(this->tcpServer, &QTcpServer::newConnection, this, [this]() {
			...
			this->tcpSocketsList.push_back(tcpSocket);//加入链表最后一个
			std::list<QTcpSocket*>::iterator it = --this->tcpSocketsList.end();//取最后一个，注意 --end() 才是最后一个 ！！
			...
			connect(tcpSocket, &QTcpSocket::disconnected, this, [this, it]() {
				...
				tcpSocket = nullptr;
				this->tcpSocketsList.erase(it);//在链表中删除
				});
			...
			connect(tcpSocket, &QTcpSocket::readyRead, this, [this, it]() {
				...
				ts << "[Tcp client " << tcpSocket->peerAddress().toString() << ' ' << tcpSocket->peerPort() << "] ";
	...
	else
	{
		if (tcpServer->isListening())
		{
			if (this->tcpSocketsList.empty() != true)//断开所有客户端
			{
				for (auto it = this->tcpSocketsList.begin(); it != this->tcpSocketsList.end(); )
				{
					QTcpSocket* tcpSocket = *it;
					if (tcpSocket != nullptr)
					{
					...
					{
						qDebug() << "错误 空指针 正常情况下不应该执行到这里 请检查 " << __FILE__ << __LINE__;
					}
					it = this->tcpSocketsList.erase(it);// erase 返回下一个迭代器
				}
			}
		...
		else
		{
			this->tcpServer->deleteLater();
			this->tcpServer = nullptr;
			qDebug() << "错误 空指针 正常情况下代码不应该执行到这里 " << __FILE__ << __LINE__ << "  请检查";
		}
	}
