...
void ReceiveWidget::showData(QByteArray data)
{
    ...
    {
        QString dataString;
        QByteArray encodingName = QByteArray::fromStdString( getEncodingString(encoding));      // 得到编码的字符串
    ...
    {
        std::vector<uint8_t> theData(data.begin(), data.end());
        QString hexString = QString::fromStdString(vectorToHexString(theData));
        stringShow += hexString;
    }

    this->insertPlainText(stringShow);
}
